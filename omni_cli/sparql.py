...
# This query is optimized to avoid run-offs: it simply gets the
...
def queryFromTemplate(template, ctx):
...
def query_epochs_by_name(name):
    ctx = {'name': name}
    result = queryFromTemplate(epochForOrchestratorQuery, ctx)
...
def get_last_run_by_name(name):
    ctx = {'name': name}
    result = queryFromTemplate(lastEpochForOrchestratorQuery, ctx)
    if result is None:
        return
    data = []
    for r in result["results"]["bindings"]:
...
def query_last_activity_by_project(name):
    ctx = {'project': name}
    result = queryFromTemplate(lastActivityForProject, ctx)
    if result is None:
        return
    data = []
    for r in result["results"]["bindings"]:
...
def query_files_for_epoch(name):
    ctx = {'project': name}
    result = queryFromTemplate(filesForEpochRun, ctx)
    if result is None:
        return
    data = []
    for r in result["results"]["bindings"]:
